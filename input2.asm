...
addi v0,zero,0
addi a0,zero,0 ; x = 0			;| On definit la head à la coord(3,4) = GSA(28)
addi a1,zero,0 ; y = 0			;|
...
addi a0,zero,0
addi s0,zero,0                  ; le score global des unités
...
addi a0,zero,0
;-- addi v0,zero,0
bne v0,t1,move ;--- if type == 1 pomme est mangée : then call createfood
addi a0,zero,1
call create_food
call draw_array
...
add t0,zero,s0 ; unité
addi t2,zero,10 ; limite
addi t0,t0,1
bge t0,t2,change_unite
addi s0,s0,1
ret
...
change_unite:
addi s0,zero,0
add t1,zero,s1 ; dizaine
addi t1,t1,1
bge t1,t2,change_dizaine
addi s1,s1,1
ret
...
;s0 est le score des unités
;s1 est le score des dizaines
